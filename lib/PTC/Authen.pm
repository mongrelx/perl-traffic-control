...
# $Source: /home/eematle/wlannet/lib/WLANnet/Authen.pm,v $
# $Revision: 1.2 $
...
$REVISION = sprintf "%d.%03d", q$Revision: 1.2 $ =~ /(\d+)/g;
$VERSION = '1.21';
...
my $conf = {
	...
	user_table	=> {
		_table	=> 'radcheck', 	# table name
		uid   	=> 'id',   	# user unique id field
		login	=> 'UserName', 	# username field
		passwd	=> 'Value',	# password field
		status	=> 'clientid',	# status field
...
sub new
{
	...
	$self->db_session($opts{db_session}) if ($opts{db_session});
	return $self;
}
...
sub login
{
	...
	{	# they've logged in before (or are spoofing)
		my $get_ticket = $self->db_session->prepare(
			'SELECT '.
...
sub _get_user_info
{
	...
	$login=~s/\@wlanmail.com//;
		...
		$self->db->quote($login).' AND Attribute="Password"'  )
...
#	my $cookie1 = new CGI::Cookie(-name=>login, -value=>login );
#        my $cookie2 = new CGI::Cookie(-name=>'ticket', 
#                                    -value=>$ticket 
#                                      );
my $query=new CGI;
$login_cookie=new CGI::Cookie(
	-name=>"login",
	-value=>"$login",
        -expires=>time+$self->expire_seconds(),
        -path=>"/",
...
print header;
print $query->header(-cookie=>[$login_cookie,$ticket_cookie]);
#print header; 

#$login_cookie= 'Set-Cookie: login=' . $login . $base_cookie . "\n";
##$ticket_cookie= 'Set-Cookie: ticket=' . $ticket . $base_cookie . "\n";
#print $login_cookie;
#print $ticket_cookie;
#print header("text/html",-cookie=>$login_cookie,-cookie=>$ticket_cookie);
